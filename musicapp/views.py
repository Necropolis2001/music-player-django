...
def index(request):
    ...
    indexpage_english_songs = Song.objects.filter(id__in=sliced_ids)
      ...
      # Display Metal Songs
    songs_metal = list(Song.objects.filter(language='Metal').values('id'))
    sliced_ids = [each['id'] for each in songs_metal][:5]
    indexpage_metal_songs = Song.objects.filter(id__in=sliced_ids)

    # Display Pop Songs
    songs_pop = list(Song.objects.filter(language='Pop').values('id'))
    sliced_ids = [each['id'] for each in songs_pop][:5]
    indexpage_pop_songs = Song.objects.filter(id__in=sliced_ids)

       # Display Rock Songs
    songs_rock = list(Song.objects.filter(language='Rock').values('id'))
    ...
    context = {
        ...
        'metal_songs':indexpage_metal_songs,
...
def rock_songs(request):
    ...
    rock_songs = Song.objects.filter(language='Rock')
...
def pop_songs(request):
    ...
    pop_songs = Song.objects.filter(language='Pop')
...
def metal_songs(request):

    metal_songs = Song.objects.filter(language='metal')
    ...
    if query:
        metal_songs = Song.objects.filter(Q(name__icontains=query)).distinct()
        context = {'metal_songs': metal_songs}
        return render(request, 'musicapp/metal_songs.html', context)

    context = {'metal_songs':metal_songs,'last_played':last_played_song}
    return render(request, 'musicapp/metal_songs.html',context=context)
